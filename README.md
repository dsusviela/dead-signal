...
Waves last 45 seconds and continue across districts, and every third wave a migration streams down a nearby avenue; neither is announced, the squad finds out on the street. The outbreak does not follow the clock: its size, speed and infected mix grow with the squad's strength (shared level, attachments, armor, turrets) and with each campaign step completed, so backtracking and exploring never make the city harder by themselves. Held interactions (preparing Blackglass, transmitting, opening the barrier) start with one press, advance while a survivor stays in reach and keep their progress if you leave; explore and backtrack in any order, and anything missing is named when you try. Boss difficulty scales with the outbreak tier and squad level you arrive with and locks when you enter. Balance and boss tuning remain in progress.
...
The original procedural dark-ambient score changes with the quarter of the city you are in (each has its own harmony and instruments) and with how much danger the squad is in: calm and suspense while you sneak, a frantic string-and-drum arrangement once you are spotted, heavier still when the horde arrives, and its own piece for the Patient Furnace. `npm run bench:music` builds a listening bench (`artifacts/music-bench.html`) that runs the real score with controls for quarter, danger and events. Each weapon has its own effect; pickups, healing, damage, reloads, revives, level-ups, waves, and the ending have audio cues. Audio starts after interaction, stops during pause, and remembers the Music and All sound preferences from Options. Everything is synthesized locally with Web Audio; no audio downloads are required.
