(function(root){
 ...
 function survivor(g,p,s,seated=false){
   ...
   if(p.armorHit>0||p.armorBroken>.6){const k=p.armorBroken>.6?(p.armorBroken-.6)/.6:p.armorHit/.35;g.globalAlpha=Math.min(1,k);g.strokeStyle=p.armorBroken>.6?'#ff8c80':'#c9e6f5';g.lineWidth=2;g.setLineDash(p.armorBroken>.6?[3,4]:[]);g.beginPath();g.ellipse(p.x,p.y-12,15,19,0,0,TAU);g.stroke();g.setLineDash([]);g.globalAlpha=1;}
